...
For example:
``` 
sh ./spark-pi.sh --master localhost:8443  --image oap-centos:1.0.0  --conf ./conf
``` 
...
For exmaple:
``` 
sh ./spark-submit.sh --master localhost:8443  --image oap-centos:1.0.0  --conf ./conf --name spark-pi --class org.apache.spark.examples.SparkPi  local:///opt/home/spark-3.0.0/examples/jars/spark-examples_2.12-3.0.0.jar 100
``` 
...
For example:
``` 
sh ./spark-thrift-server.sh start --image oap-centos:1.0.0  --conf ./conf
``` 
...
For example:
``` 
sh ./spark-client.sh start --image oap-centos:1.0.0 --conf ./conf
``` 
